...
import ro.videanuadrian.smartHome.services.listeners.TemperatureObserverSubject;
...
public class TemperatureGrabber implements Runnable,TemperatureObserverSubject {
	...
	public void run() {
		...
		while(running == true) {
			...
			float tInCelsius = (float) (1*0.0625);
			float tOutCelsius = (float) (2*0.0625);
		...
		*/
		...
		try {			
			...
			//I2CDevice insideSensor = i2cBus.getDevice(0x4a);
			//I2CDevice outsideSensor = i2cBus.getDevice(0x4b);
